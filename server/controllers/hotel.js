...
export const countByType = async (req, res, next) => {
  try {
    ...
    const countMotels = await Hotel.countDocuments({ type: 'Motel' });
    const countVillas = await Hotel.countDocuments({ type: 'Villa' });
    const countCottages = await Hotel.countDocuments({ type: 'Cottage' });

    res.status(200).json([
      { type: 'Hotel', count: countHotels },
      { type: 'Apartment', count: countApartments },
      { type: 'Motel', count: countMotels },
      { type: 'Villa', count: countVillas },
      { type: 'Cottage', count: countCottages },
    ]);
  } catch (error) {
    next(error);
  }
